    ...
    $("#notiList").on("click", function (func) {
        ...
        $.ajax({
            ...
            if (response.data.length == 0) {
                return alert("you don't have any messages!")
            }
            var  $("<table>", {
                css: {
                    "width": "520px",
...
function sendRequest(url, data, method, ifDirect) {
    $.ajax({
        ...
        success: function (response) {
            ...
            } else if (response.code == "deleteSUCCESS") {
                alert(response.message)
                location.reload();
...
function validateTrainerForm() {
    ...
    if (!valid) {
        alert("please fill out all required information in correct format")
    }
    return valid
}
...
function validateForm() {
    ...
    $.validator.addMethod("greaterThanCurrentMonthYear", function (value, element) {
        var currentDate = new Date();
        var mongh = $("#month").val()
        var inputDate = new Date(value + "-" + mongh);
    ...
    }, "Please enter a date that is equal to or greater than the current month and year.")
    var form = $("#regiForm");
    form.validate({
        rules: {
            firstname: {
            ...
            year: {
                required: true,
                greaterThanCurrentMonthYear: true
            },
            cvv: {
                number: true
            }
...
function updateMembers(type) {
    if (type == 'member') {
        if (validateForm()) {
            ...
            sendRequest('/addOrUpdateMember', formData, "POST", "form#regiForm");
        }
    } else {
